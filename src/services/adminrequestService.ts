...
export const createAdminRequest = async (
  ...
  try {
    ...
    // 400 Bad Request 등 상세 에러 메시지 처리
...
/**
 * 내 대기중인 MODERATOR 권한 요청 개수를 조회합니다.
 * @returns 대기중 요청 개수 (number)
 * @throws Error - API 실패 또는 네트워크 오류 시 예외 발생
 */
export const getPendingAdminRequestCount = async (): Promise<number> => {
  try {
    const response = await api.get<PendingAdminRequestCountResponse>('/admin-requests/pending/count');
    if (response.data.isSuccess) {
      return response.data.data;
    } else {
